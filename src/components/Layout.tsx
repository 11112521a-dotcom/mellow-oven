...
import { LayoutDashboard, Wallet, Package, ShoppingBag, Settings, Menu, TrendingUp } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange }) => {
    ...
    return (
        <div className="min-h-screen bg-cafe-50 text-cafe-900 font-sans flex">
            {/* Sidebar */}
            <aside className="w-64 bg-white border-r border-cafe-200 hidden md:flex flex-col">
            ...
            {/* Mobile Header */}
            <div className="md:hidden fixed top-0 left-0 right-0 bg-white border-b border-cafe-200 z-50 px-4 py-3 flex justify-between items-center">
                <h1 className="text-lg font-bold text-cafe-700">{storeName}</h1>
                <button className="p-2 text-cafe-600">
                    <Menu size={24} />
                </button>
            </div>
