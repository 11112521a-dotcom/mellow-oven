...
export const createPromotionSlice: StateCreator<AppState, [], [], PromotionSlice> = (set, get) => ({
    ...
    syncDeliveredOrderProfits: async () => {
        ...
        const newProfits: Array<{ date: string; amount: number; source: string }> = [];
        ...
        if (newProfits.length === 0) return 0;
        ...
        if (error) {
            throw new Error(`[Sync] Failed to batch insert unallocated profits: ${error.message}`);
        }

        if (data && data.length > 0) {
            const mappedProfits = data.map(row => ({
                id: row.id,
                date: row.date,
                amount: row.amount,
                source: row.source,
            ...
            set(s => ({
                unallocatedProfits: [...s.unallocatedProfits, ...mappedProfits]
            }));

            return mappedProfits.length;
        }
